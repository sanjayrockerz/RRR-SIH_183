...
from app.domain import Chain, ConfidenceLevel, Transfer, BridgeDefinition, AssetIdentity
...
def test_cross_chain_graph_preserves_observed_and_inferred_edges():
    ...
    assert any(node.node_id==f"tron:{TRON_C.lower()}" for node in graph.nodes)
