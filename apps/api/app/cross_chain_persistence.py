        ...
        try:
            async with self._require_pool().acquire() as conn:
                async with conn.transaction():
                    ...
                    evidence_id=uuid5(NAMESPACE_URL,f"rrr:cross-chain:evidence:{case_id}:{transfer.chain}:{transfer.tx_hash}")
                    await conn.execute("INSERT INTO evidence(evidence_id,case_id,evidence_type,chain,tx_hash,source,captured_at,metadata,created_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT (case_id,chain,tx_hash,evidence_type) DO NOTHING",evidence_id,case_uuid,"CROSS_CHAIN_TRANSFER",transfer.chain,transfer.tx_hash,transfer.provider,transfer.timestamp or now,json.dumps({"bridge_protocol":observation.bridge_contract,"destination_chain":observation.destination_chain,"destination_address":observation.destination_address}),now)
